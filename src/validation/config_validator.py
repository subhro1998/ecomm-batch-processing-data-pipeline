...
def validate_minio_config(minio_config, env: str) -> bool:
    ...
    # Validate host
    host = minio_config.get(common_constants.MINIO_HOST_CONFIG_PATH.format(env=env))
    ...
    username = minio_config.get(common_constants.MINIO_USERNAME_CONFIG_PATH.format(env=env))
    ...
    minio_password = minio_config.get(common_constants.MINIO_USER_PASSWORD_CONFIG_PATH.format(env=env))
    ...
    file_accessible = minio_config.get(common_constants.MINIO_FILE_ACCESSIBLE_CONFIG_PATH.format(env=env))
    if not isinstance(file_accessible, bool):
        logging.error("File accessible is not a boolean")
        return False

    # Validate SSL enablement config
    ssl_enabled = bool(minio_config.get(common_constants.MINIO_SSL_CONFIG_PATH.format(env=env)))
    ...
    sub_folder_template = minio_config.get(common_constants.MINIO_SUB_FOLDER_TEMPLATE_KEY)
...
def validate_data_pipeline_config(data_pipeline_config: dict,
    ...
    if (not data_pipeline_config or data_pipeline_config is None
        ...
        logging.error(f"Batch processor config file: {common_constants.DATA_PIPELINE_CONFIG_FILE} read failed, "
                      f"Stoping to process further")
        return False

    # Validate if the source system is configured
    configured_source_systems = data_pipeline_config[common_constants.CONFIGURED_SOURCE_SYSTEMS_KEY]
    ...
    configured_batch_upstream_category = data_pipeline_config[common_constants.ALL_CONFIGURED_SOURCE_CATEGORIES_KEY]
    ...
    processing_category = data_pipeline_config[
        common_constants.BATCH_SPECIFIC_CONFIG_PROCESSING_CATEGORY_KEY.format(source_system=batch_input.source_system)]
    ...
    configured_batch_run_times = data_pipeline_config[
        common_constants.BATCH_SPECIFIC_CONFIG_BATCH_RUN_TIME_KEY.format(source_system=batch_input.source_system)]
    ...
    raw_file_type = data_pipeline_config[
        common_constants.BATCH_SPECIFIC_CONFIG_RAW_FILE_TYPE_KEY.format(source_system=batch_input.source_system)]
